...
is.runGAM <- FALSE
is.runBKMR <- TRUE
is.restrict <- TRUE # Restrict data to a specific clinic.
is.loadData <- TRUE # Load bkmr object.
...
if (isTRUE(is.runGAM)) {
  ...
  g2 <- plothFuncViaGam(gam.gps, "gps", "as_ln_c")
...
if (isTRUE(is.runBKMR)) {
  ...
  if (!is.loadData) {
    ...
    path <- stringr::str_c("/Users/cathylee/Documents/Harvard/Postdoc/",
                           "EarlySigns/earlyS.progs/earlyS.Rscripts/",
                           "positivity_assessment/Archives/")
    fitted.km.standard <- get(load(stringr::str_c(path, "fitted.km.standard.RData")))
    fitted.km.gps <- get(load(stringr::str_c(path, "fitted.km.gps.RData")))
  }
  ...
  SummarizeBKMR <- function(bkmr.fit, y, Z, X) {
    # Summarize MCMC results obtained from BKMR.
    ...
    # Compare estimated h function when all predictors are at a particular
    # quantile to when all are at the 50th percentile.
    overall.risk <- bkmr::OverallRiskSummaries(fit = bkmr.fit, y = y, Z = Z,
                                               X = X)
    g1 <- ggplot(
      overall.risk,
    ...
    # Compute summaries of the risks associated with a change in a single
    # variable in Z from a 75th to a 25th percentile, for the other variables in
    # Z fixed to a specific level.
    single.risk <- bkmr::SingVarRiskSummaries(fit = bkmr.fit, y = y, Z = Z,
                                              X = X)
    g2 <- ggplot(
      filterData(single.risk),
    ...
    # Compare the single-predictor health risks when all of the other predictors
    # in Z are fixed to their 75th percentile to when all of the other
    # predictors in Z are fixed to their 25th percentile.
    interact.risk <- bkmr::SingVarIntSummaries(fit = bkmr.fit, y = y, Z = Z,
                                               X = X)
    g3 <- ggplot(
      filterData(interact.risk),
      aes(variable, est, ymin = est - 1.96 * sd, ymax = est + 1.96 * sd)) +
    ...
    univariate.h <- bkmr::PredictorResponseUnivar(fit = bkmr.fit, y = y, Z = Z,
                                                  X = X,
                                                  z.names = colnames(exposures))
    g4 <- ggplot(
      filterData(univariate.h),
  ...
  g1 <- SummarizeBKMR(fitted.km.standard, y = response, Z = exposures,
                      X = covariates)
  g2 <- SummarizeBKMR(fitted.km.gps, y = response, Z = exposures,
                      X = gps.as.covariate)
}
